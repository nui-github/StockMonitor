...
import { getChatEstimate } from "@/lib/services/chat";

const QuerySchema = z.object({ model: z.enum(["standard", "deep"]).default("standard") });

// count_tokens ไม่คิดเงิน — เรียกได้อิสระก่อนแสดง dialog ยืนยัน (docs/05 §7.2 — ใช้ pattern เดียวกับ analysis estimate)
export async function GET(req: Request, { params }: { params: Promise<{ symbol: string }> }) {
  ...
  const res = await getChatEstimate(symbol, parsed.data.model);
  if (!res.ok) {
